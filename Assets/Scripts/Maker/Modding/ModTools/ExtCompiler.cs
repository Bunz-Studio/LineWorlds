...
#region Main
        ...
        public static AssemblyInAppDomain assemblyCompiler;
        ...
        public static Assembly Compile(string directory, ExtModProject project, bool asExe = false, bool forceByProcess = false)
        {
            ...
            if (assemblyCompiler != null) assemblyCompiler.Dispose();
            assemblyCompiler = new AssemblyInAppDomain();
            var directoryPath = Path.Combine(directory, "Library");
            var assemblyPath = GetFreeAssemblyPath(directoryPath, project.PropertyGroup[0].AssemblyName, compileAttempt);
            var references = new List<string>();
            foreach (var refer in project.ItemGroup[0].Reference)
            {
                references.Add(refer.HintPath);
            }
            var output = assemblyCompiler.Compile(codes.ToArray(), references.ToArray(), assemblyPath);

            if (!output.IsError)
            {
                string location = output.Assembly.Location;
            ...
            {
                ExtActionInspector.Log("Failed to compile the assembly", "ExtCompiler", output.Exception.Message + output.Exception.StackTrace);
            }
            return null;
        }
        ...
        public static Assembly CompileLiwb(string directory, LiwbProject liwb)
        {
            ...
            if (assemblyCompiler != null) assemblyCompiler.Dispose();
            assemblyCompiler = new AssemblyInAppDomain();
            var assemblyPath = GetFreeAssemblyPath("Cache", "Assembly-CSharp", compileAttempt);
            var output = assemblyCompiler.Compile(codeData.codes.ToArray(), references.ToArray(), assemblyPath);

            // UnityEngine.Debug.Log("Compiler output: " + output);
            if (!output.IsError)
            {
                var location = output.Assembly.Location;
                // UnityEngine.Debug.Log("Trying to load assembly at: " + location + " - IsExist: " + File.Exists(location));
                var assembly = output.Assembly;
                compiledAssembly = assembly;
                AnalyzeAssembly();
