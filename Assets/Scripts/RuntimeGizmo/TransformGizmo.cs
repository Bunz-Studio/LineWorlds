...
// Credit : https://github.com/HiddenMonk/Unity3DRuntimeTransformGizmo
namespace RuntimeGizmos
{
	...
	{
		public ViewportTextureManager inputManager;
        ...
        public TransformType transformingType { get; private set; }
        ...
        public bool toggleSnapWithKey = true;
        ...
        void SnappingUpdate()
        {
            if(toggleSnapWithKey) isSnapping = Input.GetKey(translationSnapping);
        }
		...
		void OnPostRender()
		{
			...
			Color xColor = (translatingAxis == Axis.X) ? (isTransforming) ? selectedColor : hoverColor : this.xColor;
			Color yColor = (translatingAxis == Axis.Y) ? (isTransforming) ? selectedColor : hoverColor : this.yColor;
			Color zColor = (translatingAxis == Axis.Z) ? (isTransforming) ? selectedColor : hoverColor : this.zColor;
			Color allColor = (translatingAxis == Axis.Any) ? (isTransforming) ? selectedColor : hoverColor : this.allColor;

			//Note: The order of drawing the axis decides what gets drawn over what.

			TransformType moveOrScaleType = (transformType == TransformType.Scale || (isTransforming && transformingType == TransformType.Scale)) ? TransformType.Scale : TransformType.Move;
		...
		{
			TransformType transType = !checkIsTransforming || isTransforming ? transformingType : transformType;
		...
		{
			if(Input.GetKey(ActionKey) || !ExtUtility.IsValidShortcutKey()) return;
			...
			if(!isTransforming) transformingType = transformType;
		...
		void TransformSelected()
		{
			...
			{
				if(translatingAxis != Axis.None && Input.GetMouseButtonDown(0) && !EventSystem.current.IsPointerOverGameObject())
				{
		...
		IEnumerator TransformSelected(TransformType transType)
		{
			...
			var transformCommands = new List<ICommand>();
			...
			while(!Input.GetMouseButtonUp(0))
			{
				...
				//Ray mouseRay = inputManager.ScreenPointToRay(Input.mousePosition);
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					...
					{
						Vector3 projected = (translatingAxis == Axis.Any) ? transform.right : projectedAxis;
						...
						Vector3 localAxis = (GetProperTransformSpace() == TransformSpace.Local && translatingAxis != Axis.Any) ? mainTargetRoot.InverseTransformDirection(axis) : axis;
						
						Vector3 targetScaleAmount = Vector3.one;
						if(translatingAxis == Axis.Any) targetScaleAmount = (ExtVector3.Abs(mainTargetRoot.localScale.normalized) * scaleAmount);
					...
					else if(transType == TransformType.Rotate)
					{
						...
						if(translatingAxis == Axis.Any)
						{
							...
							if(circularRotationMethod)
							{
								...
								Vector3 projected = (translatingAxis == Axis.Any || ExtVector3.IsParallel(axis, planeNormal)) ? planeNormal : Vector3.Cross(axis, planeNormal);
                	...
                	ExtCore.instance.OnActionUpdate.InvokeOnExist();
				}
		...
		void GetTarget()
		{
            ...
            if(translatingAxis == Axis.None && Input.GetMouseButtonDown(0) && !ExtUtility.IsInputHoveringUI())
			{
		...
		public void AddTarget(Transform target, bool addCommand = true)
		{
			if(target != null)
			{
				...
				SetPivotPoint();
				ExtCore.instance.OnObjectUpdate.InvokeOnExist<List<GameObject>>(ExtSelection.instance.gameObjectSelection);
                ExtCore.instance.OnActionUpdate.InvokeOnExist();
            }
		}
		...
		public void ClearTargets(bool addCommand = true)
        {
            ...
            try
            {
                ExtCore.instance.OnClearObject.InvokeOnExist();
            }
            catch
            {
                Console.WriteLine("Instance no longer exist");
            }
		}
		...
		public void SetTranslatingAxis(TransformType type, Axis axis, Axis planeAxis = Axis.None)
		{
			...
			this.translatingAxis = axis;
			this.translatingAxisPlane = planeAxis;
		}
		...
		public AxisInfo GetAxisInfo()
		{
			...
			if(isTransforming && GetProperTransformSpace() == TransformSpace.Global && transformingType == TransformType.Rotate)
			{
		...
		void SetNearAxis()
		{
			...
			if(translatingAxis == Axis.None && (TransformTypeContains(TransformType.Move) || TransformTypeContains(TransformType.Scale)))
			{
				...
				if(translatingAxis == Axis.None && TransformTypeContains(TransformType.Scale))
				{
				...
				if(translatingAxis == Axis.None && TransformTypeContains(TransformType.Move))
				{
			...
			if(translatingAxis == Axis.None && TransformTypeContains(TransformType.Rotate))
			{
