...
public class CameraTrigger : MonoBehaviour
{
    ...
    {
        if (BetterCamera.current != null)
        {
            if (changeTargetObject)
            {
                BetterCamera.current.Line = targetObject;
            }
            if (setStart)
            {
                startValues.ApplyValues(BetterCamera.current);
            }
            if (setTween)
            {
                tweenValues.TweenValues(BetterCamera.current, tweenType, tweenTime, tweenDelay);
            }
        }
        else
        {
...
public class CameraValues
{
    ...
    public void TweenValues(BetterCamera cam, LeanTweenType tweenType, float tweenTime, float tweenDelay)
    {
        ...
        if (changeCameraOffset)
            LeanTween.value(cam.gameObject, cam.pivotOffset, cameraOffset, tweenTime).setEase(tweenType).setDelay(tweenDelay).setOnUpdate(
                (Vector3 result) =>
                {
                    cam.pivotOffset = result;
                }
            );
    }
}